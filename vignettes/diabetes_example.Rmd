...
Both `APES` and `mplot` were designed to have an easy-to-use user interface where an user can simply supply a `glm` object and get the analysis results rapidly. We will fit the full model here. 
...
## Variable selection under 20 bootstrap using `APES`
...
## Variable selection under 20 bootstrap using `mplot`

```{r}
t1 = Sys.time()

mplot_esult = mplot::vis(
  mf = full_model,
  B = 50,
  redundant = FALSE ## mplot adds a redundant variable by default, we will suppress this
)
...
Looking at the time differences, it is clear that APES is the winner. However, it should be noted that APES only computed the best linear model within each model size while `mplot` performed a genuine exhaustive search across all GLM. These results are not intuitively comparable. However, the results presented in [Wang et. al](https://doi.org/10.1111/anzs.12276) provide some assurance that the results of APES are good approximations to a genuine exhaustive search. 

## Comparing variable importance plot

One plot that both packages have implemented is the variable importance plot from Murray et. al. (2013). We will make a visual comparison between the two below to check if the final interpretations of these plots fit with our expectations. 
...
Based on the plots above, we see that both methods showed similar ordering of the variables as we should expect. As pointed out [by the `mplot` vignette](http://garthtarr.github.io/mplot/articles/diabetes.html), one of the most interesting feature of this data is the cross-over between some selected variables (especially the `hdl` and the `tc` variables which crosses over after after the BIC threshold) which is preserved by the `APES` package. 
