...
The APES method rely on a best subset algorithm for linear regression model. In cases where the number of regressors is less than 30, the `leaps` algorithm is sufficiently fast to provide a result. For cases where the number of regressors becomes larger, we recommend using the `bestsubset` package as the backend. However, the installation of this package is not quite straight forward. The purpose of this vignette is to detail some key steps to install this package. 
...
Follow the steps in [this article](https://cran.r-project.org/web/packages/prioritizr/vignettes/gurobi_installation.html). Specifically, you should:
...
4. Download the Gurobi software from https://www.gurobi.com/downloads/ and select the software version and operating system. Documentations specific to your operating system can be found [here](http://www.gurobi.com/documentation/). 
...
The `bestsubset` package can be installed from GitHub via the `R` command `devtools::install_github("ryantibs/best-subset/bestsubset")`.
