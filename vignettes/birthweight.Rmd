...
Looking at the p-values of the coefficients, we might suspect there are some redundant variables in the full model. So we may perform APES on this data by feeding the `full_model` to the `apes` function. 
...
We can see that when we apply some perturbation on the data, the p-value for the `ht` variable increased from 0.008 to 0.054, which could cause some confusions if we have taken a naive approach to only select variables with p-values < 0.05. Similarly, this perturbation also affects APES, as we have now selected three extra vairables under the BIC. Variable selection stability is not a criticism of any specific methods, but rather, it is an issue for data with ambiguous signals. We will describe how the `APES` package uses bootstrap sampling to improve variable selection stability in the next section. 

# Bootstrapping with APES

One way that we can improve variable selection stability is to apply `APES` on many bootstrapped versions of the data and then average the computed results. The motivation of our `APES` package is to make exhaustive selection fast and thus APES ideally suited for such a procedure that requires such large amount of computation. 

Running the `apes` function with an extra argument `n_boot` automatically invokes bootstrapping sampling on the rows of the data. The returned object is a `boot_apes` class object which stores all bootstrapped `apes` objects. This class `print`, `summary` and `plot` generic methods built for it, which makes the interaction with such an object much easier. The `summary` method summarises the averaged selection (empirical) probability of each variable for a given information criterion. In the next section, we will explore the `plot` generic methods for this object class. 
...
The variable importance plot is a technique explored in Murray et. al. (2013) and it shows the stability of each variable as empirical probability of selection against different strength of penalty on the general information criterion. The most important difference between this method and the `summary` output is that we are no longer fixated on a single information criterion like the AIC and the BIC, but rather, we can visualise the selection with ever increasing penalty term. Variables with stronger selection stability are those with high probabilities of selection with increasingly higher levels of penalisation. 
